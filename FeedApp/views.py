...
def myfeed(request):
    comment_count_list = []
    like_count_list = []
    posts = Posts.objects.filter(username=request.user).order_by("-date_posted")
    for p in posts:
        c_count = Comment.objects.filter(post=p).count()
        l_count = Like.objects.filter(post=p).count()
        comment_count_list.append(c_count)  # comment_count_list =
        like_count_list.append(l_count)  # like_count_list =
...
def friendsfeed(request):
    ...
    posts = Posts.objects.filter(username=friends).order_by("-date_posted")
    for p in posts:
        c_count = Comment.objects.filter(post=p).count()
        l_count = Like.objects.filter(post=p).count()
        comment_count_list.append(c_count)  # comment_count_list =
...
def comments(request, post_id):
    if request.method == "GET" and request.GET.get(
        "btn1"
    ):  # btn1 is the name of the button we created in the comments.html file
